...
The Johnson Neyman interval below indicates that when wellbeing is then COVID stress predicts increased music use. This is represented visually below
...
aus_data <- survey_scores %>% 
  filter(citizen_dichot == "Australian Citizen") %>% 
  mutate_all(as.numeric)

additive_music_use_aus <- lm(
  coping_strategies_2 ~ covid_scale + wellbeing_scale, 
  data = aus_data)

interacting_music_use_aus <- lm(
  coping_strategies_2 ~ covid_scale * wellbeing_scale, 
  data = aus_data)

sjPlot::tab_model(additive_music_use_aus,
                  interacting_music_use_aus, 
                  show.est = FALSE,
                  show.std = TRUE,
                  dv.labels = c("Music Use", "Music Use"))
```
...
non_aus_data <- survey_scores %>% 
  filter(citizen_dichot != "Australian Citizen") %>% 
  mutate_all(as.numeric)

additive_music_use_non_aus <- lm(
  coping_strategies_2 ~ covid_scale + wellbeing_scale, 
  data = non_aus_data)
...
sjPlot::tab_model(additive_music_use_non_aus,
                  interacting_music_use_non_aus, 
                  show.est = FALSE,
                  show.std = TRUE,
                  dv.labels = c("Music Use", "Music Use"))

non_aus_int <- broom::tidy(
  anova(
    additive_music_use_non_aus, interacting_music_use_non_aus
  )
)
...
```{r eval = FALSE}
interactions::sim_slopes(interacting_music_use_non_aus,
                         pred = covid_scale,
                         modx = wellbeing_scale,
                         robust = FALSE)
```
